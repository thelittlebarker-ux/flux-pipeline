...
function generateTrip(vehicleId, opts = {}) {
  ...
  // Consumption rises with poor skill and high speed.
  ...
  const energyUsedKwh = round((consumption / 100) * distanceKm, 2);
  ...
  const trip = {
    ...
    events,
    batteryFriendlyCharge: Math.random() < 0.5 * skill,
  };

  trip.ecoScore = scoring.tripEcoScore(trip);
  trip.co2SavedKg = co2.tripCo2SavedKg(distanceKm, energyUsedKwh);
...
function generateHistory(vehicleId, { days = 45, perDay = [0, 3], skill } = {}) {
  const trips = [];
  const now = Date.now();
  for (let d = days; d >= 0; d--) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      trips.push(generateTrip(vehicleId, { when, skill }));
    }
  }
