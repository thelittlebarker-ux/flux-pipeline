...
public/app.jsx         React dashboard source (7 views + Simulator Lab)
public/app.js          Pre-compiled output (generated by build.js — committed)
public/vendor/         Vendored React UMD builds (no CDN dependency)
public/index.html      Shell that loads the vendored React + compiled app
build.js               Compiles app.jsx -> app.js (npm run build)
data/drivev.json       Auto-created & auto-seeded on first boot
```

> The frontend has **no runtime CDN dependency and no build step at deploy time** —
> React is vendored locally and `app.js` is pre-compiled and committed. After editing
> `public/app.jsx`, run `npm run build` (needs `@babel/standalone`:
> `npm install --no-save @babel/standalone`) to regenerate `app.js`.
